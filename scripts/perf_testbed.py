...
def process_cold(args, cold_config: ColdConfig, time_data: TimeData, start_range: list, rank: int, batch_size: int, num_px: int) -> ColdResult:
    """
    ...
    cold_config.comp['batch_size'] = batch_size

    
    # Reconstruct
    cold.decode = time_wrap(cold.decode, time_data.times, 'cold_decode')
    cr.pos, cr.sig, cr.scl = cold.decode(cr.data, cr.ind, cold_config.comp, cold_config.geo, cold_config.algo, debug=args.debug, use_gpu=True)
...
def parallel_laue(comm, args, batch_size, num_px):
    """
    ...
    total_num_pix = (cold_config.file['frame'][1] - cold_config.file['frame'][0]) * (cold_config.file['frame'][3] - cold_config.file['frame'][2])
    print(f'Number of Pixels {total_num_pix}')
    ...
    cold_result, time_data = process_cold(args, cold_config, time_data, start_range, rank, batch_size=batch_size, num_px=num_px)
...
if __name__ == '__main__':
    ...
    num_px = 100000
    comm = None
    print(parallel_laue(comm, args, batch_size=1, num_px=num_px))
    print(parallel_laue(comm, args, batch_size=4, num_px=num_px))
    print(parallel_laue(comm, args, batch_size=8, num_px=num_px))
    
    print(parallel_laue(comm, args, batch_size=16, num_px=num_px))
    print(parallel_laue(comm, args, batch_size=32, num_px=num_px))
    print(parallel_laue(comm, args, batch_size=64, num_px=num_px))
    print(parallel_laue(comm, args, batch_size=128, num_px=num_px))
    print(parallel_laue(comm, args, batch_size=256, num_px=num_px))
    print(parallel_laue(comm, args, batch_size=512, num_px=num_px))
    print(parallel_laue(comm, args, batch_size=1024, num_px=num_px))
    print(parallel_laue(comm, args, batch_size=2048, num_px=num_px))
    print(parallel_laue(comm, args, batch_size=4096, num_px=num_px))
