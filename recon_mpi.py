...
def reconstruct_backup(base_path, frame, im_num, all_dir, comm):
    rank = comm.Get_rank()
    ...
    max_proc = comm.bcast(max_proc, root=0)
    avail_datasets = comm.bcast(avail_datasets, root=0)
    dims = comm.bcast(dims, root=0)
    print(f'{rank} {avail_datasets} {dims}')
    ...
    with h5py.File(os.path.join(out_dir, f'recon_{im_num}.hd5'), 'w', driver='mpio', comm=MPI.COMM_WORLD) as h5_f_out:
        h5_dsets = {}
        for ds_path in avail_datasets:
            if len(dims[ds_path]) == 1:
                h5_dsets[ds_path] = h5_f_out.create_dataset(ds_path, (im_dim, im_dim))

            elif len(dims[ds_path]) == 2:
                h5_dsets[ds_path] = h5_f_out.create_dataset(ds_path, (im_dim, im_dim, dims[ds_path][1]))

        for i in range(max_proc):
            with h5py.File(os.path.join(all_dir, f'{i}.hd5'), 'r') as h5_f_in:
                ...
                for ds_path in avail_datasets:
                    ...
                    for j, ind in enumerate(proc_ind):
                        if len(dims[ds_path]) == 1:
                            h5_dsets[ds_path][ind[0], ind[1]] = ds[j]
